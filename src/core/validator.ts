...
export async function validateUri(
  ...
  // Skip validation for Spec Kit format (more flexible, no strict front-matter requirements)
  if (entry.format === "speckit") {
    // Spec Kit files are freeform - only validate if they have explicit status
    const status = (entry.data.status || "").toString();
    if (status && STATUS[entry.kind] && !STATUS[entry.kind].includes(status)) {
      pushDiag(
        diagnostics,
        uri,
        `${entry.kind} has unknown status '${status}'`,
        0,
      );
    }
  ...
  if (entry.kind === "requirement") {
    if (!entry.data.problem)
      pushDiag(diagnostics, uri, "Requirement missing problem", 1);
    if (!Array.isArray(entry.data.metrics) || entry.data.metrics.length === 0)
      pushDiag(diagnostics, uri, "Requirement missing metrics[]", 2);
  }
  ...
  if (entry.kind === "design") {
    ...
    if (!reqId) {
      pushDiag(
        diagnostics,
        uri,
        "Design missing requirement link",
        1,
      ...
      pushDiag(
        diagnostics,
        uri,
        `Design references unknown requirement '${reqId}'`,
        1,
        vscode.DiagnosticSeverity.Error,
      );
    } else {
  ...
  if (entry.kind === "task") {
    ...
    if (!reqId) {
      ...
      pushDiag(
        diagnostics,
        uri,
        `Task references unknown requirement '${reqId}'`,
        1,
        vscode.DiagnosticSeverity.Error,
      );
    }
    ...
    if (designId) {
      ...
      if (!designEntry) {
        pushDiag(
          diagnostics,
          uri,
          `Task references unknown design '${designId}'`,
          1,
          vscode.DiagnosticSeverity.Error,
        );
        ...
        pushDiag(
          diagnostics,
          uri,
          `Design '${designId}' not approved (status=${designEntry.data.status})`,
          0,
    ...
    if (
      !Array.isArray(entry.data.acceptance) ||
      entry.data.acceptance.length === 0
    ) {
      pushDiag(
        diagnostics,
        uri,
...
export async function validateAllOpen(
  index: WorkspaceIndex,
  diagnosticCollection: vscode.DiagnosticCollection,
  onComplete?: () => void,
) {
