...
export function createStatusBar(context: vscode.ExtensionContext): vscode.StatusBarItem {
  ...
  statusBarItem.command = 'speclens.showFlowSummary';
...
export function updateStatusBar(index: WorkspaceIndex, diagnosticCollection: vscode.DiagnosticCollection) {
  ...
  statusBarItem.text = `SpecLens (R:${counts.requirement} D:${counts.design} T:${counts.task} ⚠️${diagTotal})`;
  statusBarItem.tooltip = 'SpecLens summary: Requirements / Designs / Tasks / Diagnostics';
  statusBarItem.show();
}
...
export function showFlowSummary(index: WorkspaceIndex, diagnosticCollection: vscode.DiagnosticCollection) {
  ...
  const diagTotal = diagCount(index, diagnosticCollection);
  vscode.window.showInformationMessage(`SpecLens Summary\nRequirements (${reqs.length})\nDesigns (${designs.length})\nTasks (${tasks.length})\nDiagnostics: ${diagTotal}`);
}
